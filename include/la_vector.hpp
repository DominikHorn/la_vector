...
/** Computes (bits_per_correction > 0 ? 2^(bits_per_correction-1) - 1 : 0)
 * without the conditional operator. */
#define BPC_TO_EPSILON(bits_per_correction) \
  (((1ul << (bits_per_correction)) + 1) / 2 - 1)
...
/** Computes the smallest integral value not less than x / y, where x and y must
...
template <typename K, uint8_t t_bpc = 0,
          template <class, class> class t_top_level = bucketing_top_level>
class la_vector {
  ...
  static constexpr size_t extraction_density =
      auto_bpc ? 1 : BIT_CEIL(4 * cache_line_bits / t_bpc);

  using position_type =
      typename std::conditional_t<sizeof(K) <= 4, uint32_t, uint64_t>;
  using signed_position_type = typename std::make_signed_t<position_type>;
  using larger_signed_key_type =
      typename std::conditional_t<sizeof(K) <= 4, int64_t, __int128>;
  using top_level_type =
      t_top_level<K, typename std::vector<segment>::const_iterator>;
  using canonical_segment =
      typename OptimalPiecewiseLinearModel<position_type, K>::CanonicalSegment;
  using base_segment_type =
      typename std::conditional_t<auto_bpc, variable_bpc, constant_bpc>;

  // If auto_bpc, each segment uses a different bit-size for the corrections,
  // stored in segment::bpc. It also stores segment::corrections_offset, which
  // is the cumulative sum of the preceding segments' bpc values and is used as
  // a (bit) pointer into the corrections array. If !auto_bpc, the corrections
  // array is split into two parts. The first part contains the corrections at
  // positions multiples of extraction_density. The second part contains the
  ...
  std::vector<segment> segments;  ///< The linear models that, together with the
                                  ///< corrections, compress the data.
  sdsl::int_vector<64>
      corrections;           ///< The corrections for each compressed element.
 ...
 public:
  ...
  explicit la_vector(std::vector<K> &data)
      : la_vector(data.begin(), data.end()){};
  ...
  la_vector(RandomIt begin, RandomIt end)
    ...
    for (auto it = canonical_segments.begin(); it < canonical_segments.end();
         ++it) {
      ...
      auto j = std::next(it) != canonical_segments.end()
                   ...
                   : n;
      uint8_t bpc = t_bpc;
      if constexpr (auto_bpc) bpc = it->bpc;
      push_segment(*it, bpc, corrections_offset, begin, i, j);
      corrections_offset += bpc * (j - i);
    }

    segments.emplace_back(n);  // extra segment to avoid bound checking in
                               // decode() and lower_bound()
    top_level = decltype(top_level)(begin, end, segments.begin(),
                                    std::prev(segments.end()));
  }

  /**
   * Returns the element at the specified position. No bounds checking is
   * performed.
  ...
  K operator[](size_t i) const {
    assert(i < n);
    return top_level.segment_for_position(i)->decompress(corrections.data(), n,
                                                         i);
  }
   ...
   * @return an iterator pointing to the first element that is not less than
   * value
   */
  iterator lower_bound(K value) const {
    if (value > back) return end();
    if (value <= front) return begin();
    ...
    if (!auto_bpc) {
      ...
      while (sample_lo < sample_hi) {
        ...
        if (s.decompress(corrections.data(), n, mid * extraction_density) <
            value) {
          sample_lo = mid + 1;
      ...
      if (lo == t.first) return iterator(this, t.first, std::next(it));
      return iterator(this, lo, it);
    }

    auto val = s.decompress(corrections.data(), n, pos);
    auto search_forward = val <= value;
    constexpr auto linear_threshold =
        2 * cache_line_bits / (auto_bpc ? 4 : t_bpc);
    ...
    if (hi - lo <= linear_threshold) {
      if (search_forward)
        ...
        while (pos > s.first &&
               s.decompress(corrections.data(), n, pos - 1) >= value)
          --pos;

      if (pos == t.first) return iterator(this, t.first, std::next(it));
    ...
    auto val_at_lo =
        search_forward ? val : s.decompress(corrections.data(), n, lo);
    auto val_at_hi =
        search_forward ? s.decompress(corrections.data(), n, hi - 1) : val;
    auto count = hi - lo;
  ...
  /**
   * Returns the number of elements in the container that are less than or equal
   * to @p value.
  ...
  /**
   * Decodes all the elements of this container into the memory beginning at @p
   * out. The caller is responsible for allocating enough memory for @p out,
   * that is, at least @ref size() * sizeof(K) bytes.
...
#pragma omp simd
  ...
  size_t size_in_bytes() const {
    return corrections.bit_size() / CHAR_BIT +
           segments_count() * sizeof(segment) + top_level.size_in_bytes();
  }

  /**
   * Returns the number of segments (linear models) used in this container to
   * encode its elements.
  ...
  /**
   * Returns the average number of bits per element, that is, @ref
   * size_in_bytes() * 8. / @ref size().
  ...
  size_t serialize(std::ostream &out, sdsl::structure_tree_node *v = nullptr,
    ...
    written_bytes +=
        sdsl::write_member(segments.size(), out, child, "segments.size()");
 ...
 private:
  struct canonical_segment_bpc : canonical_segment {
    ...
    canonical_segment_bpc(const canonical_segment &cs, uint8_t bpc)
        : canonical_segment(cs), bpc(bpc){};
  };

  template <typename RandomIt, bool Enable = !auto_bpc,
            typename std::enable_if_t<Enable, int> = 0>
  static std::pair<std::vector<canonical_segment>, size_t> make_segmentation(
    ...
    auto in_fun = [begin](auto i) {
      return std::pair<position_type, K>(i, begin[i]);
    };
  ...
  make_segmentation(RandomIt begin, RandomIt end) {
    ...
    auto advance_frontier = [&](auto bpc, auto target) {
      if (frontier[bpc] > target) return frontier[bpc];

      size_t i;
      for (i = frontier[bpc];
           i < n && segmentations[bpc].add_point(i, begin[i]); ++i)
        continue;
    ...
    for (size_t i = 0, next_i = -1; i < n; i = next_i, next_i = -1) {
      ...
      for (uint8_t bpc = 0; bpc <= max_bpc; bpc += 1 + (bpc == 0)) {
        ...
        if (distance[j] > distance[i] + weight_ij) {
          ...
          parent[j] = std::make_unique<canonical_segment_bpc>(
              segmentations[bpc].get_segment().copy(i), bpc);
        }
    ...
    if (!parent[n]) throw std::runtime_error("Cannot reach target vertex");
    ...
    for (size_t current = n; current != 0;
         current = parent[current]->get_first_x()) {
  ...
  template <class RandomIt>
  void push_segment(const canonical_segment &cs, uint8_t bpc,
                    position_type corrections_offset, RandomIt data, size_t i,
                    size_t j) {
    try {
      ...
      push_segment(cs.copy(half), bpc, corrections_offset + half * bpc, data,
                   half, j);
    }
  }
};
...
class la_vector<K, t_bpc, t_top_level>::constant_bpc {
 public:
...
template <typename K, uint8_t t_bpc, template <class, class> class t_top_level>
class la_vector<K, t_bpc, t_top_level>::variable_bpc {
 public:
  uint8_t bpc;
  position_type corrections_offset;
  uint32_t first_correction : 16;
  variable_bpc() = default;
  variable_bpc(uint8_t bpc, position_type offset)
      : bpc(bpc), corrections_offset(offset), first_correction(0){};
};
...
class la_vector<K, t_bpc, t_top_level>::segment : base_segment_type {
 public:
  ...
  static constexpr auto significand_bits =
      sizeof(K) <= 4 ? 32 - exponent_bits : 56 - exponent_bits;
  position_type first;
  ...
  segment(const canonical_segment &cs, uint8_t bpc,
          position_type corrections_offset, RandomIt data, size_t n, size_t i,
          size_t j, uint64_t *corrections)
      : base_segment_type(bpc, corrections_offset) {
    auto epsilon = BPC_TO_EPSILON(bpc);
    auto [cs_significand, cs_exponent, cs_intercept] =
        cs.get_fixed_point_segment(cs.get_first_x(), j - i + 1);

    if (BIT_WIDTH(cs_exponent) > exponent_bits ||
        BIT_WIDTH(cs_significand) > significand_bits)
  ...
  void set_correction(uint64_t *corrections, size_t n, size_t i,
                      uint64_t value) {
    if (this->bpc == 0) return;
    if (BIT_WIDTH(value) > this->bpc)
      throw std::overflow_error("Segment correction too large");
    if (i == first) this->first_correction = value;

    auto idx = get_correction_bit_offset(n, i);
    sdsl::bits::write_int(corrections + (idx >> 6), value, idx & 0x3F,
                          this->bpc);
  }

  K get_correction(const uint64_t *corrections, size_t n, size_t i) const {
    auto idx = get_correction_bit_offset(n, i);
    return sdsl::bits::read_int(corrections + (idx >> 6u), idx & 0x3F,
                                this->bpc);
  }

  larger_signed_key_type approximate(size_t i) const {
    return ((larger_signed_key_type(slope_significand) * (i - first)) >>
            slope_exponent) +
           intercept;
  }

  std::pair<position_type, position_type> approximate_position(
      const K &value) const {
    auto significand =
        larger_signed_key_type(slope_significand == 0 ? 1 : slope_significand);
    auto p = ((larger_signed_key_type(value) - intercept) << slope_exponent) /
                 significand +
             first;
    auto epsilon = larger_signed_key_type(BPC_TO_EPSILON(this->bpc));
    auto bound =
        1 + (epsilon << slope_exponent) / larger_signed_key_type(significand);
...
class la_vector<K, t_bpc, t_top_level>::la_iterator {
  ...
  void move_segment_cursor() {
    ...
    if constexpr (Forward) {
      if (i >= std::next(s_it)->first) ++s_it;
    } else {
      if (s_it > p->segments.begin() && i < s_it->first) --s_it;
    }
  }
 ...
 public:
  ...
  la_iterator(parent_type *p, size_t i, segments_iterator s_it)
      : p(p), i(i), s_it(s_it) {}
  la_iterator(parent_type *p, size_t i) : p(p), i(i), s_it(p->segments.end()) {
    move_segment_cursor<>();
  }

  reference operator*() const {
    return s_it->decompress(p->corrections.data(), p->n, i);
  }
  ...
  la_iterator operator+(difference_type d) const {
    return la_iterator(p, i + d);
  }
  la_iterator operator-(difference_type d) const {
    return la_iterator(p, i - d);
  }
...
ForwardIt upper_bound_branchless(ForwardIt first, ForwardIt last,
...
template <typename K, typename t_segments_iterator>
class bucketing_top_level {
  size_t val_step;  ///< The chunk size of val_top_level, in terms of universe
                    ///< values 0,...,back.
  size_t pos_step;  ///< The chunk size of pos_top_level, in terms of positions
                    ///< 0,...,n.
  size_t top_level_size;  ///< The number of elements in the two *_top_level
                          ///< structures.
  sdsl::int_vector<> val_top_level;  ///< Used to speed up segment_for_value,
                                     ///< contains positions of segments.
  sdsl::int_vector<> pos_top_level;  ///< Used to speed up segment_for_position,
                                     ///< contains positions of segments.
 ...
 public:
  ...
  bucketing_top_level(It first, It last, t_segments_iterator first_segment,
    ...
    for (size_t i = 0, j = 0, k = 0; i < top_level_size - 1; ++i) {
      while (j < n_segments &&
        ...
        ++j;
      while (k < n_segments && first_segment[k].first < (i + 1) * pos_step) ++k;
      val_top_level[i] = j;
      pos_top_level[i] = k;
    }
  }
  ...
  /**
   * Returns an iterator to the segment responsible for decompressing the
  ...
  /**
   * Returns an iterator to the segment responsible for decompressing an element
   * that is not less than the given value.
  ...
  size_t serialize(std::ostream &out, sdsl::structure_tree_node *v = nullptr,
    ...
    written_bytes +=
        sdsl::write_member(top_level_size, out, child, "top_level.size()");
    written_bytes +=
        sdsl::serialize(val_top_level, out, child, "val_top_level");
    written_bytes +=
        sdsl::serialize(pos_top_level, out, child, "pos_top_level");
    sdsl::structure_tree::add_size(child, written_bytes);
    return written_bytes;
  }
   ...
   */
  size_t size_in_bytes() const {
    return (val_top_level.bit_size() + pos_top_level.bit_size()) / CHAR_BIT;
  }
};
